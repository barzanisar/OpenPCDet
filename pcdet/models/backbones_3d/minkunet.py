...
class MinkUNet(nn.Module):
    ...
    def forward(self, batch_dict):
        x = ME.SparseTensor(features=batch_dict['voxel_features'].cpu(), coordinates=batch_dict['voxel_coords'].cpu().int(), device=torch.device('cuda'))
        ...
        point_coords = common_utils.get_voxel_centers(y4.C[:,1:], downsample_times=1, voxel_size=self.voxel_size, point_cloud_range=self.point_cloud_range)
        batch_dict['point_coords'] = torch.cat((y4.C[:,0:1].float(), point_coords), dim=1)
