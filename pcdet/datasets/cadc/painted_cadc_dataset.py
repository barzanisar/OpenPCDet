...
class PaintedCadcDataset(DatasetTemplate):
    ...
    def get_painted_lidar(self, sample_idx):
        date, set_num, idx = sample_idx
        lidar_file = os.path.join(self.root_path, date, set_num, 'labeled', 'painted_lidar_points', 'data', '%s.npy' % idx)
        assert os.path.exists(lidar_file)
        points = np.load(lidar_file) #np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)
        return points
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            info = {}
            pc_info = {'num_features': 8, 'lidar_idx': sample_idx} #TODO: painting
...
def create_cadc_infos(dataset_cfg, class_names, data_path, save_path, workers=8):
    dataset = PaintedCadcDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
