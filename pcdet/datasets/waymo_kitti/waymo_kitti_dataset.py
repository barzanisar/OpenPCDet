...
class WaymoKittiFormatDataset(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading Waymo KITTI format dataset')
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for Waymo KITTI format dataset: %d' % (len(kitti_infos)))
    ...
    def get_calib(self, idx):
        ...
        return calibration_kitti.CalibrationWaymo(calib_file)
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('waymo_kitti_dbinfos_%s.pkl' % split)
...
def create_waymo_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=1):
    dataset = WaymoKittiFormatDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, val_split = 'train', 'val'

    train_filename = save_path / ('waymo_kitti_infos_%s.pkl' % train_split)
    val_filename = save_path / ('waymo_kitti_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'waymo_kitti_infos_trainval.pkl'
    test_filename = save_path / 'waymo_kitti_infos_test.pkl'
    ...
    print('Waymo Kitti info train file is saved to %s' % train_filename)

    dataset.set_split(val_split)
    kitti_infos_val = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(val_filename, 'wb') as f:
        pickle.dump(kitti_infos_val, f)
    print('Waymo Kitti info val file is saved to %s' % val_filename)

    with open(trainval_filename, 'wb') as f:
        pickle.dump(kitti_infos_train + kitti_infos_val, f)
    print('Waymo Kitti info trainval file is saved to %s' % trainval_filename)

    # dataset.set_split('test')
    # kitti_infos_test = dataset.get_infos(num_workers=workers, has_label=False, count_inside_pts=False)
    # with open(test_filename, 'wb') as f:
    #     pickle.dump(kitti_infos_test, f)
    # print('Waymo Kitti info test file is saved to %s' % test_filename)
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_waymo_kitti_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict
        dataset_cfg = EasyDict(yaml.safe_load(open(sys.argv[2])))
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        create_waymo_kitti_infos(
            dataset_cfg=dataset_cfg,
            class_names=['VEHICLE', 'PEDESTRIAN', 'CYCLIST'],
            data_path=ROOT_DIR / 'data' / 'waymo_kitti',
            save_path=ROOT_DIR / 'data' / 'waymo_kitti'
        )
